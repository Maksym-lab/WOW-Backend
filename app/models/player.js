...
module.exports = (sequelize, DataTypes) => {
  ...
  Player.updatePlaytime = async player => {
    ...
    if (response.games) {
      ...
      const playtimeSource = response.games.find(
        game => game.appid === appIDSource
      );
      const playtimeSandstorm = response.games.find(
        game => game.appid === appIDSandstorm
      );
      if (playtimeMod) {
      ...
      console.log(
        `updated ${player.nickName}: ${JSON.stringify(columns, null, 4)}`
      );
    } else {
